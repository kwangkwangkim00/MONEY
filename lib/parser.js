...
function applyMapping(grid, mapping, sourceId, hasHeader) {
  ...
  dataRows.forEach((cols, index) => {
    ...
    // Same date+description+amount can legitimately repeat within one paste
    // (e.g. two separate coffees at the same place on the same day). Tag
    // repeats with an occurrence ordinal so they don't collide in raw_hash,
    // while re-pasting the same batch later still dedupes (ordinals recur).
    const baseHash = computeHash(sourceId, date, description, amount);
    const occurrence = (seen.get(baseHash) || 0) + 1;
    seen.set(baseHash, occurrence);
    const raw_hash = occurrence === 1 ? baseHash : `${baseHash}#${occurrence}`;
    rows.push({ date, description, amount, raw_hash });
  });
  return { rows, errors };
}
